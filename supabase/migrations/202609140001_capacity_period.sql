...
begin
 ...
 if p_action in ('movement.save','movement.delete') then
  ...
  insert into public.capacity_rules(farm_id,pasture_id,year,month,max_heads,warning_percentage,active)
  select f.id,p,extract(year from period)::integer,extract(month from period)::integer,(p_payload->>'maxHeads')::bigint,(p_payload->>'warningPercentage')::integer,coalesce((p_payload->>'active')::boolean,true)
